...
import { capOrbAlpha, contourCss, ORB_MAX_ALPHA, ORB_SHIMMER_MAX_ALPHA } from './ink';
...
function strokePath(
  ...
  ctx.strokeStyle = contourCss(dark, alpha, maximumAlpha);
  ctx.lineWidth = width;
  ctx.lineCap = 'round';
  ctx.lineJoin = 'round';
  ctx.stroke();
