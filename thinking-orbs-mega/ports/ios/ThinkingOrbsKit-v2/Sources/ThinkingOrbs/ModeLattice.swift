...
private func makeMoves(_ count: Int) -> [RubikMove] {
    ...
    for i in 0..<count {
        let fi = Double(i)
        let axis = min(2, Int(floor(orbHash(fi, 2.3) * 3)))
        let lo = -1.0 + 0.5 * min(3, floor(orbHash(fi, 5.9) * 4))
        let dir: Double = orbHash(fi, 7.7) < 0.5 ? 1 : -1
...
func drawGlobe(_ ctx: GraphicsContext, _ size: Double, _ t: Double, _ dark: Bool, _ o: OrbOpts) {
    let spin = 0.5
    let cx = size / 2
    let cy = size / 2
...
func drawRubik(_ ctx: GraphicsContext, _ size: Double, _ t: Double, _ dark: Bool, _ o: OrbOpts) {
    let cx = size / 2
    let cy = size / 2
    let R = (size / 2) * 0.82
...
func drawWave(_ ctx: GraphicsContext, _ size: Double, _ t: Double, _ dark: Bool, _ o: OrbOpts) {
    let cx = size / 2
    let cy = size / 2
