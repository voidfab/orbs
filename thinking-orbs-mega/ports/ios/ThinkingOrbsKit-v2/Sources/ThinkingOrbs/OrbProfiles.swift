...
// rows are the reference `fine` profiles; each shipped preset (state x
// size) applies count / radius multipliers on top, resolved once and
// cached — the render loop sees plain numbers.
...
func orbScaleCounts(_ opts: OrbOpts, _ scale: Double) -> OrbOpts {
    var out = opts
    var done = Set<String>()
    let rt = sqrt(scale)
...
func orbScaleRadii(_ opts: OrbOpts, _ scale: Double) -> OrbOpts {
    var out = opts
    for k in radiusKeys {
        if let v = out[k] {
            out[k] = v * scale
    ...
    // remember the multiplier itself — spacing-derived radii use it, since
...
let orbBaseProfiles: [OrbModeKey: OrbOpts] = [
    .globe: [
    ...
    .braid: [
        ...
        "rBase": 1.2, "rDepth": 1.8,
        "rsPow": 0.6, "rMin": 0.3,
    ],
    .ribbon: [
        "lanes": 5, "segs": 88, "ghostN": 150,
        "rBase": 1.1, "rDepth": 1.7,
        "rsPow": 0.6, "rMin": 0.3,
    ],
