...
export interface DotRender {
  ...
  rot?: number
  /**
   * `d` is in ball-radius units. The engine sets `dScale` to the viewBox
   * radius so paint can scale the path without touching arc flags.
   */
  dBall?: boolean
  /** viewBox scale applied to `d` at paint time; set by the engine */
  dScale?: number
}
