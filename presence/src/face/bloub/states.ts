...
export type ShapeWear = 'replace' | 'modulate' | 'character'
...
export interface StateDef {
  ...
  baseBody: boolean
  /**
   * How a customiser silhouette (ghost, nuage, …) is worn.
   * Default: `replace` when `baseBody`, else `modulate`.
   * `character` keeps the creature body and puts the state's extras on it.
   */
  shapeWear?: ShapeWear
  /**
...
export function shapeWearOf(def: StateDef): ShapeWear {
  return def.shapeWear ?? (def.baseBody ? 'replace' : 'modulate')
}

const CHARACTER_BODY = (shape: number[]): Silhouette => ({
  radii: shape,
  rot: 0,
  cx: 0,
  cy: 0,
...
/**
 * Wear a customiser silhouette on a pose. Video-faithful when `shape` is null
 * (caller skips). `replace` is the rest body; `modulate` scales the state's
 * own radii so sleep/egg/burst stay themselves, just sheet-shaped; `character`
 * keeps the creature and puts thinking dots / an alarmed face on it.
 */
function bangMark(kind: 'alert' | 'exclaim', t: number): DotRender[] {
  const italic = kind === 'alert'
  const tiltDeg = italic ? 17.7 : 0
  const tilt = (tiltDeg * Math.PI) / 180
  const travel = italic
    ? (() => {
        const p = clamp(t / 1.5)
        const go = easings.easeInOutCubic(p) * 0.82 - 0.087
        const back = t > 1.6 ? clamp((t - 1.6) / 0.4) : 0
        return (go * (1 - back) + 0.1 * back) * 0.18
      })()
    : 0
  const buzz = italic ? Math.sin(t * 2.5 * TAU) * 0.02 : 0
  const x0 = 1.16 + travel
  const y0 = italic ? -0.5 + buzz : -0.28
  const barH = italic ? 0.62 : 0.72
  const barW = italic ? 0.145 : 0.16
  const gap = 0.07
  const pointR = italic ? 0.085 : 0.095
  const along = barH / 2 + gap + pointR
  return [
    {
      x: x0,
      y: y0,
      r: barW / 2,
      d: capsulePath(barW, barH),
      rot: tiltDeg,
      opacity: 1,
      dBall: true
    },
    {
      x: x0 - Math.sin(tilt) * along,
      y: y0 + Math.cos(tilt) * along,
      r: pointR,
      opacity: 1
    }
  ]
...
export function applyShapeWear(def: StateDef, pose: Pose, shape: number[], t: number): Pose {
  ...
  if (wear === 'modulate') {
    return {
      ...pose,
      sil: { ...pose.sil, radii: pose.sil.radii.map((r, i) => r * (shape[i] ?? 1)) }
    }
  }
  if (def.id === 'thinking') {
    return {
      ...pose,
      sil: CHARACTER_BODY(shape),
      eyeAlpha: 1,
      dots: [0, 1, 2].map((i) => {
        const k = dotPulse(t, i)
        return {
          x: ([-0.4, 0, 0.4] as const)[i]!,
          y: -1.28,
          r: DOT_R * (0.72 + 0.4 * k),
          opacity: 0.5 + 0.5 * k
        }
  ...
  if (def.id === 'alert') {
    return {
      ...
      sil: CHARACTER_BODY(shape),
      eyeAlpha: 1,
      gaze: { yaw: 22, pitch: 6, roll: -8 },
      split: 17.2,
      eyes: pair(0.3, 0.58),
      dots: bangMark('alert', t),
      offX: Math.sin(t * 2.5 * TAU) * 0.018
    }
  }
  return {
    ...pose,
    sil: CHARACTER_BODY(shape),
    eyeAlpha: 1,
    gaze: { yaw: 8, pitch: -6, roll: -12 },
    split: 18.4,
    eyes: pair(0.34, 0.84),
    dots: bangMark('exclaim', t)
  }
}
